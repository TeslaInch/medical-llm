...
build_v4_dataset.py
─────────────────────────────────────────────────────────────────────────────
Loads the new scd_alpaca_clean2.jsonl data, splits 90/10, saves as 
scd_training_v4.json, and pushes to TeslaInch/SCD-Instruction-Tuning.
...
NEW_FILE = TRAIN_DIR / "scd_alpaca_clean2.jsonl"
V4_FILE = TRAIN_DIR / "scd_training_v4.json"
...
def push_to_huggingface(train: list[dict], val: list[dict], hf_token: str) -> None:
    ...
    dataset_dict.push_to_hub(
        ...
        private=False,
        commit_message=(
            f"Dataset updated with fresh {len(train) + len(val)} records from scd_alpaca_clean2."
        ),
    )
